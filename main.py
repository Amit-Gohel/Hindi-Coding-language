...
#લખો("ક ખ ગ ચ મ બ ")
def myfuc(event):
    list1 = ['$દોરવાનું દાખલ કરો$', 'દોરો.આગળ', 'દોરો.પાછળ', 'દોરો.જમણીબાજુ', 'દોરો.ડાબીબાજુ', 'દોરો.પેનનોરંગ',
             'લાલ', 'કાળો', 'વાદળી', 'લીલો', 'દોરો.વર્તુળ', 'દોરો.રદકરો', 'દોરો.પેનબંધ', 'દોરો.પેનચાલુ', 'દોરો.રંગપૂરો']
    list2 = ['લખો', 'ફરીથી', 'જો', 'અથવાજો', 'નહીતો',
             'જ્યાંસુધી', 'ની', 'શ્રેણી']  # 00ff00
    for i in range(len(list1)):
        p1 = wordcolor.colorcode(list1[i], editor)
        p1.findver()

    for i in range(len(list2)):
        p1 = wordcolor.colorcode(list2[i], editor)
        p1.findimpter()
...
editor = Text(background="#454545", foreground="#07040A",
              font=('Century schoolbook', 13, 'italic'))
editor.pack()

code_output = Text(height=10, background="#388F72", foreground="#07040A", font=(
    'Century schoolbook', 13, 'italic'))
code_output.pack()
